  ...
  return (
    <div className="pb-6">
      ...
      <div className="px-5 py-4">
        <p className="mb-4 text-sm text-abu-teks">
          Formulir permintaan pasokan besar untuk korporat dan pembeli grosir. Data akan masuk ke inbox admin koperasi.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-1">
          ...
          <FormGroup label="Nama Kontak" htmlFor="kontak">
            <Input id="kontak" value={form.kontak} onChange={(e) => update("kontak", e.target.value)} required />
          </FormGroup>
          <FormGroup label="Telepon / WhatsApp" htmlFor="telepon">
            <Input id="telepon" value={form.telepon} onChange={(e) => update("telepon", e.target.value)} required />
          </FormGroup>
          <FormGroup label="Produk Dibutuhkan" htmlFor="produk">
            <Input id="produk" value={form.produk} onChange={(e) => update("produk", e.target.value)} required />
          </FormGroup>
          <FormGroup label="Volume / Kebutuhan" htmlFor="volume">
            <Input id="volume" value={form.volume} onChange={(e) => update("volume", e.target.value)} placeholder="Contoh: 5 ton/bulan" required />
          </FormGroup>
          ...
          <FormGroup label="Tanggal Dibutuhkan" htmlFor="tanggalButuh">
            <Input id="tanggalButuh" type="date" value={form.tanggalButuh} onChange={(e) => update("tanggalButuh", e.target.value)} required />
          </FormGroup>
        ...
        <Link href="/" className="mt-4 block text-center text-sm text-abu-teks">
          ← Kembali ke landing
        </Link>
      </div>
